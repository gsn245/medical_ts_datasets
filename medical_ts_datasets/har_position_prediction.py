...
class Mimic3Mortality(MedicalTsDatasetBuilder):
    ...
    def __init__(self, **kwargs):
        ...
        Mimic3Mortality.VERSION = tfds.core.Version('1.0.{}'.format(kwargs.get('split', 1)))
        super().__init__()
    ...
    def _split_generators(self, dl_manager: tfds.download.DownloadManager):
        ...
        train_dir = join(dl_manager.manual_dir, 'in-hospital-mortality', 'data')
        train_listfile = join(dl_manager.manual_dir, 'train_listfile_{}.csv'.format(self.split))
        val_dir = train_dir
        val_listfile = join(dl_manager.manual_dir, 'val_listfile_{}.csv'.format(self.split))
        test_dir = join(dl_manager.manual_dir, 'in-hospital-mortality', 'data')
        test_listfile = join(dl_manager.manual_dir, 'test_listfile_{}.csv'.format(self.split))
